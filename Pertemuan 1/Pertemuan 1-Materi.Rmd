...
```{r}
#CATATAN
#1. kalau ingin membuat optimum, alpha 
#2. kalau holtwinter yang dimasukkan alpha
#3. kalau ses yang dimasukkan alpha untuk beta dan gammanya FALSE
#4. kalau des yang dimasukkan hanya alpha beta
#5. 
```
...
```{r}
#yt dan sales merupakan data deret waktunya. ts merupakan fungsi untuk mengubah data menjadi data deret waktu
...
points(data1.ts) #menambahkan titik pada plot
```

```{r}
ts.plot(data2.ts, xlab="Time Period ", ylab="Sales", 
        main = "Time Series Plot of Sales")
points(data2.ts) #menambahkan titik pada plot
...
training_ma <- data2[1:96,] #periode 1 sampai 96 merupakan data training
testing_ma <- data2[97:120,] #periode 97 sampai 120 merupakan data testing
train_ma.ts <- ts(training_ma$Sales) #membuat data deret waktu
test_ma.ts <- ts(testing_ma$Sales) #membuat data deret waktu
```
...
plot(data2.ts, col="red",main="Plot semua data") #kenapa tidak pakai ts.plot?karena datanya sudah merupakan data time series
points(data2.ts)
...
#data terputus karena langsung dibedakan warna, kita harus membuah satu warna terlebih dahulu baru ditimpa warna merah
```
...
Dalam hal ini nilai MAPE data latih pada metode pemulusan SMA kurang dari 2%, nilai ini dapat dikategorikan sebagai nilai akurasi yang sangat baik. Nilai MAPE yang baik adalah dibawah 10% atau 20%. Selanjutnya dilakukan perhitungan nilai MAPE data uji pada metde pemulusan SMA.

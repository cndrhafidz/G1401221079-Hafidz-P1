---
title: "Praktikum 3 MPDW - Regresi dengan Peubah Lag"
...
Data yang digunakan adalah data "Daily new confirmed COVID-19 cases & deaths per million people, World". Data ini berisi jumlah kasus baru dan kematian baru COVID-19 per juta orang di seluruh dunia setiap harinya. Data ini diambil dari [Our World in Data](https://ourworldindata.org/covid-cases).

```{r}
data <- read_xlsx("D:\\IPB\\Statistika Sem 5\\MPDW (Deret Waktu)\\pertemuan 3\\covid cleaning.xlsx", sheet="data peubah lag")
...
ggplot(data, aes(x=t, y=Yt))+
  geom_line(col="red")+
  labs(title="Daily new confirmed COVID-19 deaths per million people, World",
       x="Time", y="Deaths")+
  theme_minimal()

#plot data Xt
ggplot(data, aes(x=t, y=Xt))+
  geom_line(col="red")+
  labs(title="Daily new confirmed COVID-19 cases per million people, World",
       x="Time", y="Cases")+
  theme_minimal()
```
...
plot(data.ts, col = "red", main = "Plot Keseluruhan Data", xlab = "time")

#plot data training
plot(train.ts, col = "red", main = "Plot Data Training", xlab = "Periode")

#plot data testing
plot(test.ts, col = "red", main = "Plot Data Testing", xlab = "Periode")
```
...
$$
y_t=a(1-\lambda)+\beta\_0X_t+\beta*1Z_t+*\lambda Y{t-1}+V_t
$$
...
$$
\hat{Y_t}=(-5.232e-02)+(9.417e-01)Y_{t-1}+(2.271e-04)X_t
$$

### Peramalan dan Akurasi

Berikut adalah hasil peramalan y untuk 112 periode kedepan menggunakan model koyck

```{r}
fore.koyck <- forecast(model = model.koyck, x=test$Xt, h=112)
fore.koyck
...
$$
\hat{Y_t}=-(9.006e-02)-(4.257e-04)X_t-(2.237e-09)X_{t-1}+(3.785e-03)X_{t-2}
$$

### Peramalan dan Akurasi

Berikut merupakan hasil peramalan $y$ untuk 112 periode kedepan

```{r}
fore.dlm <- forecast(model = model.dlm, x=test$Xt, h=112)
fore.dlm
...
finiteDLMauto(formula = Yt ~ Xt,
              data = data.frame(train), q.min = 1, q.max = 6, #qmin adalah lag minimum dan qmax adalah lag maksimum
...
Dari hasil tersebut terdapat beberapa peubah yang berpengaruh signifikan terhadap taraf nyata 5%, yaitu $x_{t-6}$. Adapun keseluruhan model yang terbentuk adalah

$$
\hat{Y_t}=(-2.964e-01)-(3.948e-04)X_t+...+(3.880e-03)X_{t-6}
$$

Adapun hasil peramalan 112 periode kedepan menggunakan model tersebut adalah sebagai berikut

```{r}
#peramalan dan akurasi
fore.dlm2 <- forecast(model = model.dlm2, x=test$Xt, h=112) #h adalah berapa periode kedepan
mape.dlm2<- MAPE(fore.dlm2$forecasts, test$Yt)
#akurasi data training
GoF(model.dlm2) #goodness of fit
```
...
Hasil di atas menunjukkan bahwa peubah$x_t$, $x_{t-1}$, dan $y_{t-1}$ berpengaruh signifikan terhadap $y_t$. Hal ini ditunjukan pada hasil uji t dimana nilai-p pada peubah $\le0.05$. Model keseluruhannya adalah sebagai berikut:

$$
\hat{Y}=-0.0546877+0.0007630X_t-0.0005434X_{t-1}+0.9449810Y_{t-1}
$$

### Peramalan dan Akurasi

```{r}
fore.ardl <- forecast(model = model.ardl, x=test$Xt, h=112)
fore.ardl
```

Data di atas merupakan hasil peramalan untuk 112 periode ke depan menggunakan Model Autoregressive dengan $p=1$ dan $q=1$.

```{r}
mape.ardl <- MAPE(fore.ardl$forecasts, test$Yt) #untuk data test
mape.ardl
#akurasi data training
GoF(model.ardl) #untuk data training
```
...
Dari tabel di atas, dapat terlihat bahwa nilai AIC terendah didapat ketika $p=14$ dan $q=1$, yaitu sebesar `-193.4818`. Artinya, model autoregressive optimum didapat ketika $p=14$ dan $q=1$.
...
```{r}
par(mfrow=c(1,1))
plot(test$Xt, test$Yt, type="b", col="black", ylim=c(0.3,3.0))
